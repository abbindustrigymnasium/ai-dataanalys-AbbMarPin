...
class MainForm(QMainWindow):
    ...
    def fileButtonPressed(self):
        fileName = QFileDialog.getOpenFileName(None,caption="Select a csv Data File",directory=QtCore.QDir.currentPath())
        if len(fileName[0])>0:
            try:
                offset = 4 * 3.5 * 0 + 7
                ...
                self.data["time[s]"]               = temp["pulses"] * 1/(25/1)
                self.data["time[s]_mov"] = offset + (temp["pulses"] * 1/(25/1))
                self.data["x"] = temp["x"]
                self.data["y"] = temp["y"] * -1
                self.data["z"] = temp["z"]
